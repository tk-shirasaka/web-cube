...
$(function () {
    ...
    var Parent = React.createClass({
        ...
        router: function (data) {
            var state   = {view: viewType.init};

            if (data.length)    state = {view: viewType.page, page: data};
            if (data.forms)     state = {partsType: data};
            if (data.html)      state = {preview: data};
            if (data.Parts)     state = {view: viewType.parts, parts: data};
            if (data.Parts)     this.updateParts({Parts: {type: "Block"}, Child: data.Parts});
            this.setState(state);
        },
        render: function () {
            var listView    = [];
