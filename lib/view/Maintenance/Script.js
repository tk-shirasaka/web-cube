...
$(function () {
    var viewType    = {init: 1, page: 2, parts: 3, gallery: 4, pageAction: 5, partsAction: 6, pageForm: 7, partsForm: 8, partsAttrForm: 9, saved: 10};
    ...
    var Parts = React.createClass({
        ...
        render: function () {
            ...
            var add     = <li><ListChild data={{new: true, parent: this.props.parent}} title="add" select={this.props.select} /></li>
    ...
    var Parent = React.createClass({
        ...
        selectParts: function (parts) {
            if (parts.new) {
                this.editParts({Parts: {id: Math.random().toString().replace(".", ""), type: this.state.partsType.types[0].value, parent: parts.parent}});
            } else {
        ...
        editParts: function (parts) {
            this.setState({view: viewType.partsForm, select: parts});
        },
        ...
        router: function (data) {
            ...
            var state       = {};
            var routing     = [
                {id: viewType.init,          type: null,    next: [viewType.page]},
                {id: viewType.page,          type: "page",  next: [viewType.parts]},
                {id: viewType.parts,         type: "parts", next: [viewType.partsAction, viewType.partsForm]},
                {id: viewType.partsAction,   type: "parts", next: [viewType.partsForm, viewType.saved]},
                {id: viewType.partsForm,     type: null,    next: [viewType.partsFormAttr]},
                {id: viewType.partsFormAttr, type: null,    next: [viewType.partsForm, viewType.saved]},
            ];

            if (data && data.length)    type    = "page";
            if (data && data.forms)     type    = "form";
            if (data && data.html)      type    = "preview";
            if (data && data.Parts)     type    = "parts";
            ...
            if (type === "page")    state.page      = data;
            if (type === "form")    state.partsType = data;
            if (type === "parts" && state.view === viewType.parts) state.parts  = data;
            if (type === "parts" && state.view !== viewType.parts) state.select = data;
        ...
        render: function () {
            ...
            case viewType.page :
                ...
                break;
            case viewType.parts :
                listView.push(<div><h4>Parts List</h4><Parts data={this.state.parts.Parts} select={this.selectParts} parent={null} /></div>);
                break;
            case viewType.gallery :
                break;
            ...
            case viewType.partsAttrForm :
                ...
                listView.push(<div><h4>Parts Form</h4><Form forms={formList} change={this.changeForm} /><div className="pager"><li className="previous" onClick={this.editParts.bind(this, this.state.select)}><a href="#">Prev</a></li><button className="btn btn-primary pull-right" onClick={this.saveParts}>Save</button></div></div>);
                break;
            }
            return (
                <div>
