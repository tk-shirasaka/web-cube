    ...
    var Parent = React.createClass({
        ...
        selectAction: function (id) {
            ...
            case viewType.remove :
                state.view      = viewType.saved;
                if (type === dataType.parts) this.removeParts();
                break;
            }
        ...
        ajaxSave: function () {
            ...
            if (type === dataType.page)     url = "/maintenance/ajax_page_save";
            if (type === dataType.parts)    url = "/maintenance/ajax_parts_save";
            if (url) {
                this.routerSub({path: viewType.saved});
                this.exeAjax("POST", url, this.state.select);
            }
        },
        getNavi: function () {
            var naviList    = [];
        ...
        getAction: function () {
            var ret         = [];
            var actions     = [];
            var actionList  = [{id: viewType.edit, name: "edit"}, {id: viewType.copy, name: "copy"}, {id: viewType.remove, name: "remove"}];
            if (this.state.select.unsaved ||  this.state.parts.Parts.length)    actions = ['edit'];
            else if (this.getDataType(this.state.select) === dataType.page)     actions = ['edit', 'remove'];
            else if (this.getDataType(this.state.select) === dataType.parts)    actions = ['edit', 'copy', 'remove'];
        ...
        render: function () {
            ...
            case viewType.page :
                listView.push(<div><h4>Page List</h4><Page data={this.state.page} select={this.selectPage} add={this.addPage} /></div>);
                break;
            case viewType.parts :
                listView.push(<div><h4>{this.state.select.Page.title}</h4><Action select={this.getAction()} action={this.selectAction} /></div>);
            ...
            case viewType.actions :
                listView.push(<div><h4>{this.state.select.Parts.title}</h4><Action select={this.getAction()} action={this.selectAction} /></div>);
                break;
            case viewType.edit :
                var type    = "";
                ...
                listView.push(<div><h4>{type + ": " + this.state.select[type].title}</h4><Form forms={formList} change={this.changeForm} /><button className="btn btn-primary pull-right" onClick={this.ajaxSave}>Save</button></div>);
                break;
            }
            return (
                <div>
