...
$(function () {
    ...
    var PartsClass          = {
        ...
        _setId : function (parts) {
            var index;
            var oldId       = parts._id;
            var originalId  = parts.Parts.id;

            this._setChild(parts);
            parts._id       = (parts.Parts.parent) ? parts._id = parts.Parts.parent : "";
            parts._id      += "__" + ((parts.Parts.row) ? parts.Parts.row : "0") + ((parts.Parts.offset) ? parts.Parts.offset : "0") + Math.random().toString().replace(".", "");
            parts.Parts.id  = "Parts" + parts._id;

            index           = this.ids.indexOf(oldId);
            if (!parts._originalId && originalId) parts._originalId = originalId;
            if (index === -1) {
        ...
        setParts : function (parts, skipUpdate) {
            ...
            var parent      = this.findById(parts.Parts.parent);
            parts._id       = now;
            parts._dirty    = (this.loading) ? false : true;
            parent._dirty   = (this.loading) ? false : true;
        ...
        setPage : function(data) {
            ...
            this.exeAjax(
                ...
                data,
                function (res) {
                    PartsClass.page = res.Page;
                    for (var i = 0; i < res.Parts.length; i++) {
                        PartsClass.setParts(res.Parts[i], true);
                    }
                    PartsClass.updateHtml();
                    PartsClass.loading    = false;
                }
            );
        },
        savePage : function(data) {
            ...
            this.exeAjax(
                $saveForm,
                {Page: this.page, Parts: this.data, Removed: this.removed},
                function (res) {
                    PartsClass.setPage({page: PartsClass.getPage().id});
                    partsRefresh();
                }
            );
        },
        ...
        updateHtml : function() {
            this._rebuild();
            var InputOption, parts;
            var parentInput = $partsForm.find("[name='Parts.parent']");
            var ids         = PartsClass.getIds(this.base);
            var data        = [this.findById(this.base)];

            $(parentInput).find("option").remove();
            $(parentInput).append($("<option>").val("").text("None"));
            for (var i = 0; i < ids.length; i++) {
                parts   = PartsClass.findById(ids[i]);

                if (!parts || parts.Attr.child === undefined) continue;
                $(parentInput).append($("<option>").val(parts._id).text(parts.Parts.title));
            }
    ...
    var PartsGallery        = React.createClass({
        render: function () {
            var gallery    = PartsClass.getGallery();
            return (
    ...
    var PartsControl        = React.createClass({
        ...
        partsCopy: function () {
            var parts       = copyObject(PartsClass.findById(this.state.now), ["id", "_id", "_originalId"]);
        ...
        updateList: function (id) {
            ...
            var parent      = (parts) ? parts.Parts.parent : "";
            this.setState({now : id, parent: parent});
        },
        render: function () {
            var parts       = PartsClass.findById(this.state.now);
            var listGroup   = "list-group-item";
            var partsOnly   = (parts) ? listGroup : "hide";
            var hasChild    = (parts && parts.Attr.child !== undefined) ? listGroup : "hide";
    ...
    var PartsContainer      = React.createClass({
        getInitialState: function () {
            return {ids: [], html: "", page: PartsClass.getPage(), isOk: false, gallery: false};
        },
        pageSave:   function () {
            putFormData($saveForm, {Page: PartsClass.getPage()});
    ...
    function init() {
        ...
        $partsForm.submit(function (e) {
            ...
            e.preventDefault();
            PartsClass.setParts($.extend((parts) ? parts : {}, getFormData(this)));
