...
$(function () {
    var viewType    = {page: 1, parts: 2, gallery: 3, pageForm: 4, partsForm: 5};
    var actionType  = {add: 1, edit: 2, copy: 3, remove: 4};
    var formType    = {input: 1, number: 2, checkbox: 3, raddio: 4, select: 5, textarea: 6};
    var columnList  = (function () { var ret = []; for (var i = 1; i <= 12; i++) { ret.push({value: i, name: i}); }; return ret; });
    var size        = {
    ...
    var Form = React.createClass({
        render: function () {
            var child = this.props.forms.map(function (form) {
                var formTag = "";
                switch (form.type) {
                case formType.input :
                    formTag = <input name={form.name} value={form.value}></input>;
                    break;
                case formType.number :
                    formTag = <input name={form.name} type="number" value={form.value}></input>;
                    break;
                case formType.select :
                    formTag = <select name={form.name}>{form.options.map(function (option) { return <option value={option.value}>{option.name}</option>})}</select>;
                    break;
                case formType.textarea :
                    break;
                }
                return <div><label>{form.label}{formTag}</label></div>;
            }, this);
    ...
    var Action = React.createClass({
        render: function () {
            var actionList  = [
                {id: actionType.add, name: "add"},
                {id: actionType.edit, name: "edit"},
                {id: actionType.copy, name: "copy"},
                {id: actionType.remove, name: "remove"},
            ];
    ...
    var Parts = React.createClass({
        ...
        render: function () {
            var child   = this.props.data.map(function (child) {
                ...
                var children    = (child.Child && this.state.open.indexOf(id) >= 0 ) ? <Parts data={child.Child} select={this.props.select}/> : "";
    ...
    var Parent = React.createClass({
        ...
        listPage: function () {
            this.exeAjax("GET", "/maintenance/ajax_page_list");
        },
        selectAction: function (id) {
            ...
            case actionType.edit :
                this.editParts(this.state.select);
                break;
        ...
        editParts: function (now) {
            this.setState({view: viewType.partsForm});
        },
        copyParts: function () {
        ...
        getNavi: function () {
            ...
            if (this.state.view === viewType.partsForm) naviList.push({title: "Parts Edit", state: {view: viewType.partsForm}});
            return naviList;
        },
        ...
        render: function () {
            ...
            case viewType.parts :
                ...
                if (!this.state.select) listView.push(<div><h4>Parts List</h4><Parts data={this.state.parts.Parts} select={this.selectParts} /></div>);
                break;
            case viewType.gallery :
                break;
            ...
            case viewType.partsForm :
                formList.push({type: formType.input, name: "Parts.title", label: "Title", value: (this.state.select) ? this.state.select.Parts.title : ""});
                formList.push({type: formType.input, name: "Parts.class", label: "Class", value: (this.state.select) ? this.state.select.Parts.class : ""});
                formList.push({type: formType.number, name: "Parts.row", label: "Row", value: (this.state.select) ? this.state.select.Parts.row : ""});
                formList.push({type: formType.select, name: "Parts.col", label: "Col", options: columnList(), value: (this.state.select) ? this.state.select.Parts.col : ""});
                formList.push({type: formType.select, name: "Parts.offset", label: "Offset", options: columnList(), value: (this.state.select) ? this.state.select.Parts.offset : ""});
                listView.push(<Form forms={formList} />);
                break;
            }
            return (
                <div>
