...
$(function () {
    ...
    var upIconClass         = "pull-right glyphicon glyphicon-chevron-up";
    var downIconClass       = "pull-right glyphicon glyphicon-chevron-down";
            ...
            var list = this.props.ids.map(function (id) {
                ...
                return (
                    <button id={id} type="button" className="list-group-item" style={isChild}>
                        Title : {title}
                        <span className={hasChild + " hide-child " + downIconClass}></span>
                        <span className={hasChild + " show-child " + upIconClass}></span>
                    </button>
                );
            });
            return (
    ...
    function partsEdit() {
        ...
        $("#PartsList button").bind("click", function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (!hidden && id.split("__").length + 1 < ids[i].split("__").length) continue;
                children.push("#" + ids[i]);
            }
            if (children.length) {
                if (hidden) {
                    $(children.join()).slideUp();
                } else {
                    $(children.join()).slideDown();
                    $(children.join()).children(".show-child").show();
                    $(children.join()).children(".hide-child").hide();
                }
                $(this).children((hidden) ? ".hide-child" : ".show-child").hide();
                $(this).children((hidden) ? ".show-child" : ".hide-child").show();
                $(children.join()).removeClass("shown");
            }
