...
$(function () {
    ...
    var PartsControl        = React.createClass({
        render: function() {
            ...
            var partsOnlyClass  = listGroupClass + " parts-only";
            var list = this.props.ids.map(function (id) {
                var parts       = PartsClass.findById(id);
                var title       = (parts.Parts.title) ? parts.Parts.title : "--";
                var isChild     = {display: (parts.Parts.parent && !(PartsClass.base && PartsClass.base === parts._id)) ? "none" : "block"};
                var hasChild    = (parts.Child.length) ? "" : "hide";
                return (
                    <button id={id} type="button" className="list-group-item" style={isChild}>
                        Title : {title}
                        <span className={hasChild + " hide-child " + slideDownIconClass}></span>
                        <span className={hasChild + " show-child badge"}>{parts.Child.length}</span>
                    </button>
                );
            });
            return (
                <div className="col-xs-3 col-md-3 col-sm-3">
                    <div className="panel panel-default">
                        ...
                        <ul id="ActionList" className="list-group">
                            <button id="PageEdit"      type="button" className={listGroupClass}>Page Select  <span className={selectIconClass}></span></button>
                            <button id="PartsNew"      type="button" className={listGroupClass}>New Parts    <span className={addIconClass}></span></button>
                            <button id="PartsEdit"     type="button" className={partsOnlyClass}>Edit Parts   <span className={editIconClass}></span></button>
                            <button id="PartsCopy"     type="button" className={partsOnlyClass}>Copy Parts   <span className={copyIconClass}></span></button>
                            <button id="PartsRemove"   type="button" className={partsOnlyClass}>Remove Parts <span className={removeIconClass}></span></button>
                            <button id="PartsAddChild" type="button" className={partsOnlyClass}>Add Child    <span className={addIconClass}></span></button>
                            <button id="PartsZoomIn"   type="button" className={partsOnlyClass}>Zoom In      <span className={zoomInIconClass}></span></button>
                            <button id="PartsZoomOut"  type="button" className={listGroupClass}>Zoom Out     <span className={zoomOutIconClass}></span></button>
                        </ul>
                    </div>
                    <div className="panel panel-default">
    ...
    function pageSelect() {
        $("#PageEdit").unbind("click");
        $("#PageEdit").bind("click", function () {
            $("#PageFormParent").modal();
        });
    }

    function partsEdit() {
        $("#PartsList .hide-child").each(function () {
            if (!$(this).parent().hasClass("shown")) $(this).hide();
        });

        $("#PartsEdit").unbind("click");
        $("#PartsEdit").bind("click", function () {
            var parts       = PartsClass.findById(now);
        ...
        $("#PartsList button").unbind("click");
        $("#PartsList button").bind("click", function () {
            var id          = $(this).attr("id");
            var parts       = PartsClass.findById(id);
            var children    = "";
            var hidden      = $(this).hasClass("shown");

            $("#ActionList .parts-only").slideDown();
            $(".parts-active").removeClass("parts-active");
            $("#PartsList button").removeClass("active list-group-item-warning");
            $("#Parts" + id).addClass("parts-active");
            $(this).addClass("active");
            $(this).toggleClass("shown");

            while (parts.Parts.parent) {
                $("#" + parts.Parts.parent).addClass("list-group-item-warning");
                parts       = PartsClass.findById(parts.Parts.parent);
            }
            children        = "#" + PartsClass.getIds(id, hidden ? 0 : 1).join(",#");
            children        = children.replace("#" + id + ",", "");
            if (hidden && "#" + id !== children) {
                $(children).slideUp();
            } else {
                $(children).slideDown();
                $(children).children(".show-child").show();
                $(children).children(".hide-child").hide();
            }
            $(this).children((hidden) ? ".hide-child" : ".show-child").hide();
            $(this).children((hidden) ? ".show-child" : ".hide-child").show();
    ...
    function partsCopy() {
        $("#PartsCopy").unbind("click");
        $("#PartsCopy").bind("click", function () {
            var parts       = copyObject(PartsClass.findById(now), ["_id", "_originalId"]);
    ...
    function partsRemove() {
        $("#PartsRemove").unbind("click");
        $("#PartsRemove").bind("click", function () {
            if (now) PartsClass.remove(now);
        });
    }

    function partsAdd() {
        $("#PartsNew, #PartsAddChild").unbind("click");
        $("#PartsNew").bind("click", function () {
            now = null;
            $("#PartsFormParent").modal();
        });
        $("#PartsAddChild, #PartsNew").bind("click", function () {
            putFormData($partsForm, {"Parts.parent": now});
            $("#PartsFormParent").modal();
            now = null;
        });
    }
    ...
    function partsZoom() {
        if (!PartsClass.base)   $("#PartsZoomOut").addClass("hide");
        if (PartsClass.base)    $("#PartsZoomOut").removeClass("hide");
        $("#PartsZoomIn, #PartsZoomOut").unbind("click");
        $("#PartsZoomIn").bind("click", function () {
            PartsClass.zoom(now);
        });
        $("#PartsZoomOut").bind("click", function () {
