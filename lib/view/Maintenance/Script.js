...
$(function () {
    ...
    var PartsClass          = {
        ...
        _setId : function (parts) {
            ...
            var prefix      = "__" + ((parts.Parts.row) ? parts.Parts.row : "0") + parts.Parts.offset;
            parts._id       = (parts.Parts.parent) ? parts._id = parts.Parts.parent : "";
            parts._id      += prefix + Math.random().toString().replace(".", "");
        ...
        setPartsRecursive : function (parts, parent) {
            parts.Parts.parent  = parent;
            parts._originalId   = (parts.Parts.id) ? parts.Parts.id : null;
            this._setId(parts);

            for (var i = 0; i < parts.Child.length; i++) {
                this.setPartsRecursive(parts.Child[i], parts._id);
            }
            if (!parent) this._setParts(parts);
        },
            ...
            this.exeAjax(
                ...
                function (res) {
                    ...
                    for (var i = 0; i < res.length; i++) {
                        PartsClass.setPartsRecursive(res[i]);
                    }
                ...
                return (
                    <div className="btn-group" style={isChild}>
                        ...
                        <ul id={parts._id} className="dropdown-menu">
                            ...
                            <li className="parts-show-btn"><a href="#">Show Child <span className={partsControlDown}></span></a></li>
                            <li className="parts-hide-btn"><a href="#">Hide Child <span className={partsControlUp}></span></a></li>
                        </ul>
                    </div>
                );
            });
    ...
    function partsCopy() {
        ...
        $(".parts-container .parts-control").find(".parts-copy-btn").bind("click", function () {
            var id          = $(this).parent().attr("id");
            var parts       = PartsClass.findById(id);
            PartsClass.setPartsRecursive(copyObject(parts, ["_id", "_originalId"]));
            PartsClass.updateHtml();
            console.log(parseObject(PartsClass.data))
        });
    }
    ...
    function copyObject(obj, excludes) {
        ...
        jQuery.each(parsed, function (key, val) {
            ...
            for (var i = 0, key = keys[0]; i < keys.length - 1; i++, key = keys[i]) {
                if (!obj[key]) obj[key] = {};
                obj = obj[key];
            }
