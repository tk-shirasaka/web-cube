    ...
    var Form = React.createClass({
        render: function () {
            var child = this.props.forms.map(function (form, index) {
                ...
                switch (form.type) {
                case formType.input :
                    formTag = <input id={index} className="form-control" name={form.name} value={form.value}></input>;
                    break;
                case formType.number :
                    formTag = <input id={index} className="form-control" name={form.name} type="number" value={form.value}></input>;
                    break;
                case formType.select :
                    formTag = <select id={index} className="form-control" name={form.name}>{form.options.map(function (option) { return <option value={option.value}>{option.name}</option>})}</select>;
                    break;
    ...
    var Parent = React.createClass({
        ...
        render: function () {
            ...
            case viewType.partsForm :
                formList.push({type: formType.hidden, name: "Parts.id", label: null, value: (this.state.select) ? this.state.select.Parts.id : Math.random().toString().replace(".", "")});
                ...
                listView.push(<div><h4>Parts Form</h4><Form forms={formList} /><div className="pager"><li className="next"><a href="#">Next</a></li></div></div>);
                break;
            }
            return (
                <div>
