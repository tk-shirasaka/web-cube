...
$(function () {
    var viewType    = {init: 1, page: 2, parts: 3, gallery: 4, pageAction: 5, partsAction: 6, pageForm: 7, partsAdd: 8, partsForm: 9, partsAttrForm: 10, saved: 11};
    var dataType    = {any: -1, form: 1, preview: 2, page: 3, parts: 4, new: 5};
    ...
    var Parent = React.createClass({
        ...
        addParts: function (parent) {
            this.router({new: true, type: dataType.parts, parent: parent});
        },
        copyParts: function () {
        ...
        removeParts: function () {
            ...
            this.selectPage(this.state.parts.Page.id);
        },
        updateParts: function (parts) {
            this.exeAjax("POST", "/maintenance/ajax_parts_render", parts);
        },
        saveParts: function () {
            this.exeAjax("POST", "/maintenance/ajax_parts_save", this.state.select);
            this.selectPage(this.state.parts.Page.id);
        },
        ...
        getDefaultData: function (data) {
            var id      = data.type + "-" + Math.random().toString().replace(".", "");
            var ret     = {};

            switch (data.type) {
            case dataType.parts :
                ret = {Parts: {id: id, type: this.state.partsType.types[0].value, parent: data.parent}, Attr: {}};
                break;
            }
            return ret;
        },
        ...
        router: function (data) {
            var curr, next, nextList = [];
            var type        = dataType.any;
            var state       = {};
            var routing     = [
                {id: viewType.init,             type: dataType.any,     next: [viewType.page]},
                {id: viewType.page,             type: dataType.page,    next: [viewType.parts]},
                {id: viewType.parts,            type: dataType.parts,   next: [viewType.partsAction, viewType.partsAdd]},
                {id: viewType.partsAction,      type: dataType.parts,   next: [viewType.partsForm, viewType.saved]},
                {id: viewType.partsAdd,         type: dataType.new,     next: [viewType.partsAttrForm]},
                {id: viewType.partsForm,        type: dataType.any,     next: [viewType.partsAttrForm]},
                {id: viewType.partsAttrForm,    type: dataType.any,     next: [viewType.partsForm, viewType.saved]},
            ];

            if (data && data.length)    type    = dataType.page;
            if (data && data.forms)     type    = dataType.form;
            if (data && data.html)      type    = dataType.preview;
            if (data && data.Parts)     type    = dataType.parts;
            if (data && data.new)       type    = dataType.new;

            routing.map(function (val) { if (val.id === this.state.view) curr = val; }, this);
            routing.map(function (val) { if (curr.next.indexOf(val.id) >= 0) nextList.push(val); });
            nextList.map(function (val) { if (val.type === type) next = val; });

            if (type === dataType.new)      data            = this.getDefaultData(data);
            if (next)                       state.view      = next.id;
            if (type === dataType.page)     state.page      = data;
            if (type === dataType.form)     state.partsType = data;
            if (type === dataType.new)      state.select    = data;
            if (type === dataType.parts && state.view === viewType.parts) state.parts  = data;
            if (type === dataType.parts && state.view !== viewType.parts) state.select = data;
            if (state.view === viewType.parts)  this.updateParts({Parts: {type: "Block"}, Child: data.Parts});
            this.setState(state);
        },
        render: function () {
            var listView    = [];
            ...
            case viewType.pageForm :
                break;
            case viewType.partsAction :
                listView.push(<div><h4>Action List</h4><Action view={viewType.parts} select={this.state.select} action={this.selectAction} /></div>);
                break;
            case viewType.partsAdd :
            case viewType.partsForm :
                ...
                listView.push(<div><h4>Parts Form</h4><Form forms={formList} change={this.changeForm} /><div className="pager"><li className="next" onClick={this.router}><a href="#">Next</a></li></div></div>);
                break;
            case viewType.partsAttrForm :
                this.state.partsType.forms[this.state.select.Parts.type].map(function (form) {
                ...
                listView.push(<div><h4>Parts Form</h4><Form forms={formList} change={this.changeForm} /><div className="pager"><li className="previous" onClick={this.router}><a href="#">Prev</a></li><button className="btn btn-primary pull-right" onClick={this.saveParts}>Save</button></div></div>);
                break;
            }
            return (
                <div>
