...
$(function () {
    ...
    var actionType  = {edit: 0, copy: 1, remove: 2};
    var size        = {
    ...
    var Action = React.createClass({
        render: function () {
            ...
            case viewType.page :
                break;
            case viewType.parts :
                actionList  = ["edit", "copy", "remove"];
                break;
            case viewType.gallery :
                break;
            }
            var child = actionList.map(function (child, index) {
                return <li key={index}><span className={icons.get(child)}></span><ListChild data={index} title={child} select={this.props.select} /></li>;
            }, this);
            return <ul className="animated slideInLeft">{child}</ul>;
        }
    });
    ...
    var Parts = React.createClass({
        ...
        render: function () {
            var child   = this.props.data.map(function (child) {
                ...
                var toggle      = (child.Child) ? <span className={icons.get((this.state.open.indexOf(id) >= 0) ? "slideUp" : "slideDown")} onClick={this.toggleChild.bind(this, id)}></span> : "";
                return <li key={id}>{toggle}{badge}<ListChild data={child} title={title} select={this.props.select} />{children}</li>;
            }, this);
            return <ul className="animated slideInLeft" style={{margin: "10px 0 10px"}}>{child}</ul>;
        }
    });
    ...
    var Parent = React.createClass({
        ...
        selectAction: function (id) {
            ...
            case actionType.remove :
                state = {select: null, parts: this.removeParts()};
                break;
            }
            this.setState(state);
        },
        ...
        render: function () {
            ...
            case viewType.page :
                listView = <Page data={this.state.page} select={this.selectPage} />;
                break;
            case viewType.parts :
                if (this.state.dirty) this.updateParts({Parts: {type: "Block"}, Child: this.state.parts.Parts});
                listView = (this.state.select) ? <Action view={this.state.view} select={this.selectAction} /> : <Parts data={this.state.parts.Parts} select={this.selectParts} />;
                break;
            case viewType.gallery :
                break;
            }
            return (
                <div>
                    ...
                    <div className={size.get(9)} dangerouslySetInnerHTML={{__html: preview}} />
                </div>
            )
        }
    });
