...
$(function () {
    ...
    var PartsClass          = {
        ...
        setPartsRecursive : function (parts, parent, skipSetParts) {
            parts.Parts.parent  = parent;
            parts._originalId   = (parts.Parts.id) ? parts.Parts.id : null;
            this._setId(parts);

            for (var i = 0; i < parts.Child.length; i++) {
                this.setPartsRecursive(parts.Child[i], parts._id, true);
            }
            if (!skipSetParts) this._setParts(parts);
        },
    ...
    function partsCopy() {
        ...
        $(".parts-container .parts-control").find(".parts-copy-btn").bind("click", function () {
            var id          = $(this).parent().attr("id");
            var parts       = PartsClass.findById(id);
            PartsClass.setPartsRecursive(copyObject(parts, ["_id", "_originalId"]), parts.Parts.parent);
            PartsClass.updateHtml();
        });
    }
